...
class CTGAN(Base_Model):
  ...
  def init_loss(self, opt):
    ...
    # feature metric loss Projection Loss  RestructionLoss(opt.map_projection_loss)
    self.criterionFea = torch.nn.L1Loss().to(self.device)

    # map loss
    self.criterionMap = Map_loss(opt.map_projection_loss).to(self.device)
  ...
  #fea_m_lambda = self.opt.feature_D_lambda   num_D: 3  map_n_layers_D: 3
  #self.criterionFea = torch.nn.L1Loss().to(self.device)
  def feature_metric_loss(self, D_fake_pred, D_real_pred, loss_weight, num_D, feat_weights, criterionFea):
    ...
    #print('len(D_fake_pred) - 1:',len(D_fake_pred[0]) - 1)
    #print('D_fake_pred[0]:',D_fake_pred[0])
    #print('isinstance:',isinstance(D_fake_pred[0], list))
    if isinstance(D_fake_pred[0], list):
      for di in range(num_D):
        for i in range(len(D_fake_pred[di])):
          loss_G_fea += weight * criterionFea(D_fake_pred[di][i], D_real_pred[di][i].detach()) * fea_m_lambda
    # single discriminator
    else:
      for i in range(len(D_fake_pred)):
  ...
  def backward_G(self):
    ...
    #idt_loss: mse
    if idt_lambda > 0:
      # focus area weight assignment 三面之一面？
    ...
    # feature metric loss 
    #fea_m_lambda = feature_D_lambda: 0-10.0
    if fea_m_lambda > 0:
      self.loss_fea_m = self.criterionAuxiliary(self.G_fake_D, self.G_real_D) * fea_m_lambda
      #self.loss_fea_m = self.feature_metric_loss(D_fake_pred, D_real_pred, loss_weight=fea_m_lambda, num_D=self.opt.num_D, feat_weights=4.0 / (self.opt.n_layers_D + 1), criterionFea=self.criterionFea)

    # map loss Projection loss
    #map_m_lambda = map_projection_lambda: 10
    #map_projection_loss: l1——k1
    #map_m_type: l1
    if map_m_lambda > 0:
      self.loss_map_m = 0.
      #这是一个循环，迭代处理self.multi_view中的每个方向（或者视图方向）。self.multi_view可能包含多个视图的标识
      for direction in self.multi_view:
        #这是一个函数调用，用于计算损失函数的值，可能是两个图像之间的损失。它通常用于度量生成的图像与真实图像之间的相似性或差异性。
        self.loss_map_m += self.criterionMap(
          #这是一个动态属性访问，根据direction的值构建属性名称，然后获取生成的假图像。这可能是生成器模型生成的模型假图像。
          #同样，这也是一个动态属性访问，用于获取与direction相关联的真实图像。
          getattr(self, 'G_Map_{}_fake'.format(direction)),
          getattr(self, 'G_Map_{}_real'.format(direction))) * map_m_lambda
        #这可能是为了对多视图损失的贡献进行权衡，可以根据需要增加或减少损失的权重。
      self.loss_map_m = self.loss_map_m / len(self.multi_view)

    # auxiliary loss 三面之其他两面？
    ##auxiliary_lambda: 0.0——10.0
    ##auxiliary_loss: mse
    if self.auxiliary_loss:
      self.loss_auxiliary = (self.criterionAuxiliary(self.G_fake_D1, self.G_real_D1) +
                             self.criterionAuxiliary(self.G_fake_D2, self.G_real_D2)) * self.opt.auxiliary_lambda
