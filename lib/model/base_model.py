...
class Base_Model(nn.Module):
  ...
  def set_requires_grad(self, nets, requires_grad=False):
    ...
    for net in nets:
      if net is not None:
        #这是一个嵌套的循环，用于迭代处理net模型中的所有参数。net.parameters()通常返回模型中所有的可学习参数（权重和偏差）
        for param in net.parameters():
          #requires_grad是一个布尔值，如果设置为True，表示该参数需要计算梯度用于反向传播；如果设置为False，表示该参数不需要计算梯度
          #通过在训练和推理阶段动态地设置参数的requires_grad属性，可以控制哪些参数在反向传播中被更新，哪些参数保持不变
    ...
    for name in self.loss_names:
      if isinstance(name, str):
        ...
        if torch.is_tensor(loss_value):
          errors_ret[name] = float(loss_value.cpu())
        else:
