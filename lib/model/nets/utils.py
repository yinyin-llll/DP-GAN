...
def get_generator_activation_func(opt, activation_type='relu'):
  if activation_type == 'relu':
    ...
    #opt=None
    #DoubleBlockLinearUnit：这是一个函数或类，通常用于构建神经网络的激活层
    #low=opt.dblu[0], high=opt.dblu[1], k=opt.dblu[2]
    #这些是参数，通过将它们传递给 functools.partial，可以在创建 activation_layer 时固定它们的值。
