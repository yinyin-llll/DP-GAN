...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
                 ...
                 act_layer=nn.GELU, style_dim=512,norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        # Double Attn
        shortcut = x
        #print('x.transpose(-1, -2):',x.transpose(-1, -2).shape)
        x = self.norm1(x)
        #print('x:',x.shape)
...
class StyleBasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C  = x.shape
        #print('StyleBasicLayer.shape:',x.shape)
        x=x.reshape(B, C, int(L**0.5), -1)
        return x
