...
class NLayer_3D_Discriminator(nn.Module):
  def __init__(self, input_nc, ndf=64, n_layers=3,
    ...
    sequence = [[
      #nn.Conv3d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw),
      nn.Conv3d(input_nc, input_nc, kernel_size=kw, stride=2, padding=padw,groups=input_nc),
      nn.Conv3d(input_nc, ndf, kernel_size=1, padding=0),
      nn.LeakyReLU(0.2, True)
    ]]
    ...
    for n in range(1, n_layers):
      ...
      sequence += [[
        ...
        nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult_prev,kernel_size=kw, stride=2, padding=padw, bias=False,groups=ndf * nf_mult_prev),  
        nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult,kernel_size=1, padding=0, bias=use_bias),        
        norm_layer(ndf * nf_mult),
        nn.LeakyReLU(0.2, True)]]
    ...
    sequence += [[
      ...
      nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult_prev,kernel_size=kw, stride=1, padding=padw, groups=ndf * nf_mult_prev,bias=False),
      nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult,kernel_size=1, padding=0, bias=use_bias),          
      norm_layer(ndf * nf_mult),
      nn.LeakyReLU(0.2, True)
    ]]
    ...
    if use_sigmoid:
      sequence += [[
        nn.Conv3d(ndf * nf_mult, n_out_channels, kernel_size=kw, stride=1, padding=padw),

                    nn.Sigmoid()]]
    else:
      sequence += [[
        #nn.Conv3d(ndf * nf_mult, n_out_channels, kernel_size=kw, stride=1, padding=padw)
        nn.Conv3d(ndf * nf_mult, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw,groups=ndf * nf_mult),
        nn.Conv3d(ndf * nf_mult, n_out_channels, kernel_size=1, padding=0)
                    ]]
